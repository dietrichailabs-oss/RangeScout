"""Exhaustive R7 canonical-identity and provider-route audit."""
...
def _database() -> Path:
    database = Path(tempfile.mkdtemp(prefix="rangescout-r7-exhaustive-")) / "history.sqlite"
...
def main() -> int:
    ...
    for index, row in enumerate(rows, 1):
        ...
        try:
            ...
            expected_asset = canonical_asset_class(
            ...
            research = plan_research(instrument.asset_class, instrument.subtype)
            if not research.route.value or not research.visible_sections:
    ...
    payload = {
        "schema": "rangescout.r7-exhaustive-instrument-audit.v1",
