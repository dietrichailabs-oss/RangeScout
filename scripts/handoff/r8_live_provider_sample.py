"""Recorded-seed R8 fresh 30+ semantic and provider validation sample."""
...
from scripts.handoff.r8_exhaustive_instrument_audit import _provider_plan


SEED = "RangeScout-R8-live-provider-20260824"
EXCLUDED_PRIOR = {
    "FTHF", "PENN", "QBUF", "MPB", "SIO", "SEZL", "FLC", "NGHT", "CINF", "FWONK",
    "NNRGF", "BBMC", "PMAR", "MOD", "SBFMW", "TROW", "FIW", "BA", "NAT",
    "GLU$A", "REGCP", "LDP", "NLY$G", "VSA", "YB", "GRVY", "LAFAU", "DLNG$A", "KRP",
    "BGLWW", "AISPW", "BBCQW", "SDHIR", "CAPNR", "MYXXR", "ECC-PD", "NMCO", "CBXL", "CRBN",
    "BGC", "HOFT", "CHAR", "MTLK", "LGCXF",
}
TARGETS = {
    "preferred": 4,
    "cef_preferred": 2,
...
def _database() -> Path:
    path = Path(tempfile.mkdtemp(prefix="rangescout-r8-live-sample-")) / "history.sqlite"
...
def _bucket(row: sqlite3.Row) -> str:
    ...
    if row["issuer_entity_type"] == "closed_end_fund" and row["security_role"] == "preferred_security":
        return "cef_preferred"
...
def main() -> int:
    ...
    for row in rows:
        ...
        if (symbol not in EXCLUDED_PRIOR or reference_adversarial) and bucket in buckets:
    ...
    payload = {
        "schema": "rangescout.r8-live-provider-sample.v1",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "selection_seed": SEED,
        "population": len(rows),
        "exclusions": {"prior_r6_and_r7_samples": sorted(EXCLUDED_PRIOR), "unique_reference_reuse": ["BTC/USD", "EUR/USD", "^DJI", "^IXIC", "^GSPC", "XAU/USD"]},
